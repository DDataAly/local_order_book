...
@pytest.fixture
def small_order_book(small_order_book_snapshot):
    return OrderBook(small_order_book_snapshot)
...
class TestOrderBookInit:
    ...
    @pytest.mark.skip(reason='not relevant; should initialise with correct values')
...
class TestExtractOrderBookBidsAsks:
    ...
    async def test_returns_dict(self):
        ...
        test_order_book = OrderBook(test_snapshot)
    ...
    @pytest.mark.asyncio
    async def test_parses_snapshot_correctly(self,small_order_book, small_order_book_snapshot, index, expected_output):
            result = await small_order_book.extract_order_book_bids_asks(small_order_book_snapshot)
            assert result[index] == expected_output    


#     @pytest.mark.skip('it is a valid test for validate_snapshot not the extraction function')
